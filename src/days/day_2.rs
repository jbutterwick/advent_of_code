...
impl From<&RPSMove> for RPSResult{
    fn from(rps_move: &RPSMove) -> Self {
        match rps_move {
            ...
            _ => panic!("didn't recognize RPS Result")
        }
    }
}
    ...
    fn from_game(game: &str) -> Option<RPSGameP1> {
        ...
        match split.collect::<Vec<&str>>()[..] {
            [elf, you] => {
                ...
                let result = (your_move.get_result(&elf_move), elf_move.get_result(&your_move));
                Some(
                    RPSGameP1 {
                        elf:elf_move,
                        you: your_move,
...
pub(crate) fn run() -> Option<(String,String)> {
    ...
    Some((part1_total_score.to_string(),part2_total_score.to_string()))
}